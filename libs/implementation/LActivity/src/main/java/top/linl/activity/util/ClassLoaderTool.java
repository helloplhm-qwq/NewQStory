...
public class ClassLoaderTool {
    ...
    private static Class<?> getBaseTypeClass(String baseTypeName) {
        ...
        throw new RuntimeException(baseTypeName + " <-不是基本的数据类型");
    }
    ...
    public static Class<?> getClass(String className) {
        if (!LOADER_IS_INIT.get())
            throw new RuntimeException("宿主类加载器未初始化 请先使用 setHostClassLoader方法初始化 !");
        //类缓存里有这个类就直接返回
    ...
    public static void setHostClassLoader(ClassLoader loader) {
        if (loader == null) throw new RuntimeException("类加载器为Null 无法设置");
        try {
            loader.loadClass(ClassLoaderTool.class.getName());
        } catch (Throwable e) {
            HostLoader = loader;
            LOADER_IS_INIT.set(true);
            return;
        }
        throw new RuntimeException("想要设置的类加载器不是宿主的 !");
    }
